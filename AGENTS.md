...
**Multi-project MCP:** when one `mcp-adjutant` process serves multiple Cursor workspaces, pass `workspace_root` (absolute project path) on every tool call except `query_job_status`. Do not hardcode a single project in `~/.cursor/mcp.json` via `MCP_ADJUTANT_PROJECT_ROOT` / `cwd`.
...
**Blueprint plan → execute:** invoke skill [`.cursor/skills/adjutant-blueprint/`](.cursor/skills/adjutant-blueprint/SKILL.md) or MCP tools `plan_blueprint` → `execute_blueprint`. Premium evaluates Planner then BlueprintExecutor; do not hand-apply SEARCH/REPLACE when execute applies.
