...
mod tests {
    ...
    #[test]
    fn parse_write_test_suite_arguments_errors_on_missing_field() {
        let err = parse_write_test_suite_arguments(&json!({
            "path": "tests/example.rs",
            "content": "#[test] fn t() {}",
        }))
        .expect_err("missing tdd_phase should error");

        assert_eq!(err, "tool argument 'tdd_phase' must be a string");
    }

    #[test]
    fn parse_components_extracts_string_array() {
        let components = parse_components(&json!({
            "components": ["auth::middleware", "db::UserRepository"],
        }))
        .expect("components");
    ...
    #[test]
    fn parse_components_errors_when_missing_array() {
        let err = parse_components(&json!({})).expect_err("missing components should error");
        assert_eq!(
            err,
            "tool argument 'components' must be an array of strings"
        );
    }

    #[test]
    fn parse_components_errors_when_array_contains_non_strings() {
        let err = parse_components(&json!({ "components": ["ok", 42] }))
            .expect_err("non-string entries should error");
        assert_eq!(err, "tool argument 'components' must contain only strings");
    }

    #[test]
    fn parse_factory_arguments_extracts_struct_and_file() {
        let (target_struct, target_file) = parse_factory_arguments(&json!({
            "target_struct": "User",
            "target_file": "src/models/User.java",
        }))
        .expect("factory args");

        assert_eq!(target_struct, "User");
        assert_eq!(target_file, "src/models/User.java");
    }

    #[test]
    fn parse_factory_arguments_errors_on_missing_target_file() {
        let err = parse_factory_arguments(&json!({ "target_struct": "User" }))
            .expect_err("missing target_file should error");
        assert_eq!(err, "tool argument 'target_file' must be a string");
    }
}
