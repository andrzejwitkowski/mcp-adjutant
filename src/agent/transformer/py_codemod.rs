...
pub fn try_python_call_codemod(
    snippet: &str,
    instruction: &str,
    file_path: &Path,
) -> Option<String> {
    ...
    let mut has_source_module = snippet.contains("source_module=");
    let mut out = Vec::new();
    ...
    for line in snippet.lines() {
            ...
            && trimmed.contains('(')
        {
            ...
            out.push(format!("{field_indent}source_module=\"{source_module}\","));
            has_source_module = true;
            changed = true;
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn renames_sort_event_call_fields() {
        let snippet = "    log_sort_event(\n        SortEvent(\n            headline=SortHeadline(\n                component=\"bubble\",\n                message=\"sorted\",\n            ),\n            meta=SortMeta(\n                tags=[\"demo\"],\n                correlation_id=None,\n            ),\n        )\n    )";
